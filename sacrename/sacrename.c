...
int main(int argc, char **argv)
{
  ...
  for(i=1;i<argc;i++)
	...
	if (i+1>=argc) dumpError("Not enough parameters to proceed.\n");
	if (argv[i+1][0]!='/') dumpError("Directory name should be an absolute path.\n");
	defaultdir = argv[i+1];
	i++;
	copy=1;
	continue;
	...
	if (nfilelist !=0  ) dumpError("You should use -d or -f\n");
	if (i+1>=argc) dumpError("Not enough parameters to proceed.\n");
	if (argv[i+1][0]=='-') dumpError("Directory name should not begin with '-'.\n");
	dir = argv[i+1];
	i++;
	continue;
      }
	...
	if (dir!=NULL) dumpError("You should use -d or -f\n");
	if (i+1>=argc) dumpError("Not enough parameters to proceed.\n");
	while ((i+1)<argc)
	  {
	    if(argv[i+1][0]=='-') break;
  ...
  if (dir != NULL && nfilelist==0)
    {
      fprintf(stderr,"Using directory: %s for search for files with ending: %s\n",dir,fileend);
      i=ftw(dir,loadfilelist,20);
      if (i!=0){
	dumpError("Error loading file list.\n");

      }
      fprintf(stderr,"Discovered %d file%s.\n",nfilelist,(nfilelist==1)?"":"s");
      fprintf(stderr,"\n");
    }

  /* Checking for files to process */
  if (nfilelist==0) dumpError("No files, no work.\n");
  ...
  if (defaultdir!=NULL)
    {
      while (defaultdir[strlen(defaultdir)-1]=='/') defaultdir[strlen(defaultdir)-1]='\0';
      if (!isdir(defaultdir)) 
	dumpError("Default directory doesn't exist or, isn't a directory.\n");
  ...
  if (stop!=0 && realgo!=1)
    dumpError("Will not proceed without --luke flag, errors have been found!\n");


  fprintf(stderr,"Stopping before proceed, hit Ctrl-C if you are unsure, proceding in:\n");
  for(i=5;i>0;i--)
    {
      fprintf(stderr,"%ds ",i);
      sleep(1);
  ...
  if (copy==0)
	...
	{
	  if (filelist[i].ready!=1) continue;
      ...
      if (j==0) fprintf(stderr," %d from %d files renamed correctly.\n",count,nfilelist);
...
void showHelp()
{
  ...
  fprintf(stderr,"  sacrename [-h] [-v] [--luke] [--showme] [-c] [-e <fileend>] [-m <dirbase>] <-d <dir>|-f <file 1> ... <file N>>\n\n");

  fprintf(stderr,"\t-h) Show help.\n");
  fprintf(stderr,"\t-v) Show version.\n");
  fprintf(stderr,"\t--luke) Use the force luke (do it at your own risk).\n");
  ...
  fprintf(stderr,"\t-m) Change the location of renamed files, imply -c.\n");
