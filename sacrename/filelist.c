...
void addFilename(const char *fpath)
{
  ...
  filelist[nfilelist].filename=malloc(sizeof(char)*(strlen(fpath)+1));
  if (filelist[nfilelist].filename==NULL) return;
...
void showfilelist()
{
  ...
  for(i=1;i<nfilelist;i++)
    if (strlen(filelist[i].filename) > max) max=strlen(filelist[i].filename);
  ...
  for(i=0;i<nfilelist;i++)
    if (filelist[i].ready)
      fprintf(stderr,fmt,i,filelist[i].filename,filelist[i].newfilename);
    else
...
int checkfilelist()
{
  ...
  for(i=0;i<nfilelist;i++)
    {
      if (filelist[i].ready==0) continue;
	...
	{
	  if (filelist[j].ready==0) continue;
...
int createname(FILES *f)
{
  ...
  if (defaultdir == NULL)
    {
      f->newfilename = malloc(sizeof(char)*(strlen(dir)+strlen(kstnm)+25+1));
      if (f->newfilename==NULL) return -1;
	 ...
	 34 + strlen(station) + strlen(mainbasename) + 1 */
      f->newfilename = malloc(sizeof(char)*(strlen(defaultdir)+strlen(kstnm)+34+1));
      if (f->newfilename==NULL) return -1;
      sprintf(f->newfilename,
