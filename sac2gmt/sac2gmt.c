...
int main(int argc, char **argv)
{
    ...
    for(i=1;i<argc;i++) {
        ...
        if ( (j = strncmp(argv[i],"-i",2)) == 0  && i < argc-1){
            config.ninfo++;
            config.info = realloc(config.info, sizeof(char*)*config.ninfo);
        ...
        if ( (j = strncmp(argv[i],"-f",2)) == 0 && i < argc-1){
            ...
            config.nlist++;
            config.list = realloc(config.list, config.nlist*sizeof(FILELIST));
    ...
    if (config.refvar != NULL){
        ...
        if (sd == NULL || sd->type != TYPE_FLOAT){
            fprintf(stderr,"Invalid reference variable -- should be of type FLOAT\n");
            stop = 1;
        }
    }
    ...
    if (config.sortvar != NULL){
        ...
        if (sd == NULL || sd->type != TYPE_FLOAT) {
            fprintf(stderr,"Invalid sorting variable -- should be of type FLOAT\n");
            stop = 1;
        }
    }
...
int preprocess(const char *fpath, const struct stat *sb, int typeflag) {
    ...
    if (
            (typeflag == FTW_F) &&
            ((i=strncasecmp(&fpath[strlen(fpath)-strlen(config.fileend)],config.fileend,strlen(config.fileend))) == 0)
            ) {
        config.nlist++;
        config.list = realloc(config.list, config.nlist*sizeof(FILELIST));
    ...
    if (config.ninfo>0){
        for(i=0; i<config.ninfo; i++){
            if (!isMarkField(config.info[i])) {
                char *string = hd_showValueFromChar(hdr,config.info[i],NULL,NULL,NULL);
                if (string == NULL) {
                    fprintf(stderr,"Variable %s is not defined in file %s", config.info[i], f->path);
                ...
                float mark;
                hdu_getValueFromChar (config.info[i], hdr, &mark, NULL, NULL);
...
void showHelp()
{
    ...
    fprintf(stderr,"  sac2gmt [-h] [-v] [-e <file ending string>] <-d <dir> | <-f filename> [-u] [-r <head>] [-s <head>] [-i <head>]\n");
    ...
    fprintf(stderr,"\t-r) Use header variable as time origin on conversion\n");
    fprintf(stderr,"\t-s) Sort traces by this header variable\n");
    fprintf(stderr,"\t-i) Add this variable on the info line, line begging with '>' (use more than one to get more variables in)\n");
