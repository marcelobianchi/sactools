...
events *newEventList(defs * d, stations * ss)
{
	...
	for (j = 0; j < d->glb->gl_pathc; j++) {
		...
		if (ev->n != 0) {
			addev(evs, ev);
			if (v)
				fprintf(stdout, "\r%s: [A] [%03d] \n",
						d->glb->gl_pathv[d->currentdir], ev->n);
		} else {
			if (v)
				fprintf(stdout, "\r%s: [D] [%03d] \n",
						d->glb->gl_pathv[d->currentdir], ev->n);
		}
...
void EXAZ_process(glob_t * glb, char *stname)
{
	...
	for (i = 0; i < n; i++) {
		...
		float x =
			fabs(plist[i]->residual) * sin(plist[i]->baz * M_PI / 180.0);
		float y =
			fabs(plist[i]->residual) * cos(plist[i]->baz * M_PI / 180.0);
...
void EXHIST_process(glob_t * glb)
{
	...
	while (ch != 'q') {
		...
		long totaln = 0;
		for (panel = 0;
			 ((panel < d->max)) && ((panel + d->offset)) < ss->n;
			 panel++) {
			sta = d->offset + panel;
			...
			if (plist == NULL) {
				...
				if (v)
					fprintf(stderr, "Hist %d (%s %s)\n", sta,
							ss->slist[sta]->net, ss->slist[sta]->name);
				makehistorgram(plist, n, -4, 4, 31, 0);
