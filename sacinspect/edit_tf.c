...
float minmaxrefine(tf * f, defs * d, float ax, float halfmaxdist)
{
	...
	while (status == 0 && w < halfmaxdist) {
		...
		if (hdu_checkNPTS(f->hz, current))
			return ax;
		if (hdu_checkNPTS(f->hz, Nstart))
			return ax;
		if (hdu_checkNPTS(f->hz, Nend))
			return ax;
		if (Nend <= Nstart)
			return ax;
...
void timedraw(g_ctl * ctl, tf * f, defs * d, int gravity)
{
	...
	if (f->hz->a != SAC_HEADER_FLOAT_UNDEFINED)
		mark(ctl, f->hz->a, "a", 2);
	if (f->hz->f != SAC_HEADER_FLOAT_UNDEFINED)
		mark(ctl, f->hz->f, "f", 2);
