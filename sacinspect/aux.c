...
void plot (g_ctl * ctl, float *y, int npts, float delta, float b, int overlay, int color, float zoom)
{
  float *x = NULL;
  int i;
  int oldcolor;
...
void filtertf(tf *f, defs *d) {
  ...
  if (d->filter) {
    ...
    f->zf = iir (f->z, f->hz->npts, f->hz->delta,
		 (d->hp > 0.0) ? 2 : 0, d->hp, (d->lp > 0.0) ? 2 : 0, d->lp);
...
void multitraceplot (defs *d)
{
  ...
  lastFile = ((d->offset + d->max) >= d->nfiles)?(d->nfiles - 1):(d->offset + d->max);
  x1 = d->files[d->offset].hz->a + d->files[d->offset].reference - d->gpre;
  x2 = d->files[lastFile].hz->a + d->files[lastFile].reference + d->gpost;
  ...
  for (j = 0; j < d->max && (j + d->offset) < d->nfiles; j++) {
    ...
    if (d->onlyselected && !this->selected) continue;      
	...
	if (d->files[0].hz->o != SAC_HEADER_FLOAT_UNDEFINED) {
	  ...
	  ctl_xreset_mm (ctl[j], x1  - d->files[d->offset].hz->o, x2 - d->files[lastFile].hz->o);
	} else {
    ...
    if (this->selected == 1) color = 2; // If event Selected Color = Red
    // if (this->hz->unused25 != 1) color = 14; // If we are in the restricted mode and trace is locked Color = Gray
    if (d->overlay) color = j+1;

    // Finally Plot
    plot (ctl[j], (d->filter && this->zf !=NULL)?this->zf:this->z, this->hz->npts, this->hz->delta, start, 0, color, d->zoom);
    /* *********************************************************** */
    ...
    if (d->plotsearchsize){
      mark (ctl[j],this->hz->a + start - this->hz->b+d->searchsize , "", 3);
      mark (ctl[j],this->hz->a + start - this->hz->b-d->insetsearchsize , "", 3);
    }
  ...
  sprintf(string,"[%03d-%03d/%03d] [Filter %s (%.2f to %.2f hz)]",d->offset+1,
	   d->offset + d->max, d->nfiles, (d->filter)?"ON":"OFF", d->hp, d->lp
	  );
  cpgmtxt ("B", 2.5, 0.0, 0.0, string);
  
  sprintf (string,"[%c] [PSw: %.1f ISw: %.1f Tr/S: %d] [%s]", aligChar, d->searchsize, d->insetsearchsize, d->max,(d->sortmode==0)?"DZ":"BAZ");
  cpgmtxt ("B", 2.5, 1.0, 1.0, string);
  
  plothelp(d);

  ctl_resizeview (ctl[d->max-1]);
  sprintf(string,"%11s",(d->currentdir-1>=0)?d->glb->gl_pathv[d->currentdir-1]:"First Directory");
  cpgmtxt ("T", 1.0, 0.35, 1.0, string);
  
  sprintf(string,"%11s",(d->currentdir+1<d->glb->gl_pathc)?d->glb->gl_pathv[d->currentdir+1]:"Last Directory");
...
void checkTREF(tf *files, int nfiles) {
  ...
  for(i = 0; i < nfiles; i++)
    if (files[i].hz->a == SAC_HEADER_FLOAT_UNDEFINED) has = 0;
  
  if (has == 1) return;

  lerchar("Not all your files has a pick, enter the name of header to import picks or none to quit?", phase, 4);
  ...
  for(i = 0; i < nfiles; i++) {
    ...
    if (val==SAC_HEADER_FLOAT_UNDEFINED){
      strcpy(message,"The desired header variable is also undefined.");
      alert(WARNING);
      continue;
    }
...
tf *inputme (int argc, char **argv, defs *d) {
  ...
  for (i = 0; i < argc; i++) {
    ...
    loadtf(&files[i]);    
    if (files[i].z == NULL) return NULL; 
  }
  
  for (i = 0; i < argc; i++) {
    if(files[i].hz->user0 != -12345.0 && files[i].hz->user0 > files[i].hz->user1) {
...
void tffree(tf *tf, int n) {
  ...
  for(i = 0; i < n; i++){
    ...
    if (tf[i].zf != NULL) free(tf[i].zf);
    if (tf[i].z  != NULL) free(tf[i].z);
    if (tf[i].hz  != NULL) free(tf[i].hz);
...
void loaddefaults() {
  ...
  home=getenv("HOME");
  if (home != NULL) sprintf(filename,"%s/.sacinspectrc",home);
