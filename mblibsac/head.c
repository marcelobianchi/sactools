...
char *hd_showValueFromChar(SACHEAD *h, char *var, char *fmtf, char *fmti,char *fmts)
{
  ...
  if (fmtf==NULL) fmtf=default_float;
  if (fmti==NULL) fmti=default_int;
  if (fmts==NULL) fmts=default_char;
...
void showHeadElements(char *tab)
{
  ...
  for(i=0;i<SAC_HEADER_FIELDS;i++)
   {
     fprintf(stderr,"%s%10s type: %3d CharSize: %3d",tab,x[i].name,x[i].type,x[i].charsize);
     if ((i+1)%perc != 0) fprintf(stderr," | ");
     if ((i+1)%perc == 0) fprintf(stderr,"\n");
   }
