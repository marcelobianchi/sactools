...
int process(char *filename) {
  ...
  char class[2] = { 'C', '\0'};
  rms_before = rms(data, hdr, picktime-inset-25.0, picktime-inset-5.0);
  ...
  if (rms_before*2.2 < rms_hit) {	  
    ...
    rms_after  = rms(data, hdr, picktime+onset / 2, picktime+onset / 2 + (onset+inset));

    if (rms_hit > rms_after) class[0] = 'A';
  }
...
int main(int argc, char **argv) {
  ...
  for (i = 1; i < argc; i++) {	  
    ...
    if (dir == NULL && filename == NULL && i < argc-1){
      ...
      fprintf(stderr,"E:> You already specified a dir, cannot take a file.\n");
      stop = 1;
    }
  ...
  if (input == NULL) stop = fprintf(stderr,"E:> No Input Mark.\n");  
