		...
		if ((j = strncmp(argv[i], "--", 2)) == 0) {
			...
			while (i < argc) {
				...
				if (blacklisted(var[n])) {	/* Check if we can change it */
					fprintf(stderr,
							"Variable '%s' should not be changed.\n",
							var[n]);
					stop = 1;
				}
				if (i < argc) {
...
void showHelp()
{
	...
	fprintf(stderr,
			"  sacchhdr [-h] [-v] [--vars] [-e <file ending string>] <-f <file>|-d <dir>> <<-- <var> <value> [<var> <value> .... ]> | <-oz> | <-on> | <-oe> | <-ogmt> >\n");
	...
	fprintf(stderr,
			"\t-oz) Set orientation to vertical (cmpinc=0, cmpaz=0 and kcmpnm=z).\n");
	fprintf(stderr,
			"\t-on) Set orientation to NS (cmpinc=90, cmpaz=0 and kcmpnm=n).\n");
	fprintf(stderr,
			"\t-oe) Set orientation to EW (cmpinc=90, cmpaz=90 and kcmpnm=e).\n");
}
