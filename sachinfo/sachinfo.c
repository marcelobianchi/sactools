...
int process(char *filename)
{
  ...
  if (h!=NULL){
    if (fullprint && noname==0) fprintf(stdout,"%s\t",filename);
  } else {
    if (fullprint) fprintf(stdout,"Error reading file (%s)]\n",filename);
    return 1;
  }
...
void showHelp()
{
  ...
  fprintf(stderr,"  sachinfo [-h] [-v] [--vars] [-e <file ending string>] <-d <dir> <var1> ... <varN>|<file> <var1> ... <varN>\n");
